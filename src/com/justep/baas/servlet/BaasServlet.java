...
public class BaasServlet extends HttpServlet {
	...
	private static void initLog4j(ServletConfig config) {
		String home = System.getenv(JUSTEP_HOME_NAME);
		String log_home = config.getServletContext().getRealPath("/");
		if (home == null) {
		...
		try {
			...
			if (f.exists()){
				log_home = f.getCanonicalPath() + "/logs";
			}
		} catch (IOException e) {
			//忽略错误路径
		}
		
		System.setProperty(LOG_HOME_NAME, log_home);//设置日志的缓存目录
		PropertyConfigurator.configure(config.getServletContext().getRealPath(LOG_CONFIG));//初始化log4j
	}
